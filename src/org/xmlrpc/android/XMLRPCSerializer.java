...
class XMLRPCSerializer {
    ...
    static void serialize(XmlSerializer serializer, Object object) throws IOException {
        ...
        if (object instanceof String) {
        	//如果包含emoji表情则该句回报错，因为java这个序列化对象不支持，后面考虑换其他对象
        ...
        if( object instanceof Media ) {
            ...
            String chunk = null;
            //int ctr = 0;
            //Log.i("MyBaby", "converting media file to base64");
            while ((length = inStream.read(buffer)) > 0) {
                chunk = Base64.encodeToString(buffer, 0, length, Base64.DEFAULT);
                serializer.text(chunk);
                //ctr+=3600;
                //Log.i("MyBaby", "chunk " + ctr);
            }
            //Log.i("MyBaby", "conversion done!");
            inStream.close();
    ...
    //因为XmlSerializer不支持emoji表情序列化，因此用字符串处理的
    @SuppressWarnings("unchecked")
	static String serialize(Object object) throws IOException {
        ...
        // check for scalar types:
        if (object instanceof Integer || object instanceof Short || object instanceof Byte) {
        	serializer+=getStartTag(TYPE_I4)+object.toString()+getEndTag(TYPE_I4);
        } else
        if (object instanceof Long) {
        	serializer+=getStartTag(TYPE_I8)+object.toString()+getEndTag(TYPE_I8);
        } else
        if (object instanceof Double || object instanceof Float) {
        	serializer+=getStartTag(TYPE_DOUBLE)+object.toString()+getEndTag(TYPE_DOUBLE);
        } else
        if (object instanceof Boolean) {
            Boolean bool = (Boolean) object;
            String boolStr = bool.booleanValue() ? "1" : "0";
            serializer+=getStartTag(TYPE_BOOLEAN)+boolStr+getEndTag(TYPE_BOOLEAN);
        } else
        if (object instanceof String) {
        	serializer+=getStartTag(TYPE_STRING)+escapedString(object.toString())+getEndTag(TYPE_STRING);	
        } else
        if (object instanceof Date || object instanceof Calendar) {
            Date date = (Date) object;
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd'T'HH:mm:ss");
        ...
        if (object instanceof byte[] ){
            ...
            try {
                value = Base64.encodeToString((byte[])object, Base64.DEFAULT);
                serializer+=getStartTag(TYPE_BASE64)+value+getEndTag(TYPE_BASE64);
            } catch (OutOfMemoryError e) {
                throw new IOException("Out of memory");
            }
        }
            ...
            //Log.i("MyBaby", "conversion done!");
            inStream.close();
            serializer+=getEndTag("base64");
        }else
        if (object instanceof List<?>) {
        	serializer+=getStartTag(TYPE_ARRAY)+getStartTag(TAG_DATA);
            List<Object> list = (List<Object>) object;
            Iterator<Object> iter = list.iterator();
            while (iter.hasNext()) {
                Object o = iter.next();
                serializer+=getStartTag(TAG_VALUE);
                serializer+=serialize(o);
                serializer+=getEndTag(TAG_VALUE);
            }
            serializer+=getEndTag(TAG_DATA)+getEndTag(TYPE_ARRAY);
        } else
        if (object instanceof Object[]) {
        	serializer+=getStartTag(TYPE_ARRAY)+getStartTag(TAG_DATA);
            Object[] objects = (Object[]) object;
        ...
        if (object instanceof Map) {
            serializer+=getStartTag(TYPE_STRUCT);
            Map<String, Object> map = (Map<String, Object>) object;
            Iterator<Entry<String, Object>> iter = map.entrySet().iterator();
            while (iter.hasNext()) {
                Entry<String, Object> entry = iter.next();
                ...
                serializer+=getStartTag(TAG_MEMBER);
                serializer+=getStartTag(TAG_NAME)+key+getEndTag(TAG_NAME);
                serializer+=getStartTag(TAG_VALUE);
                serializer+=serialize(value);
                serializer+=getEndTag(TAG_VALUE);
                serializer+=getEndTag(TAG_MEMBER);
            }
            serializer+=getEndTag(TYPE_STRUCT);
        } else {
            throw new IOException("Cannot serialize " + object);
        }
        return serializer;
        ...
        if (typeNodeName.equals(TYPE_DATE_TIME_ISO8601)) {
           // dateFormat.setCalendar(cal);    2014-12-13 不要根据时区转换时间，由使用的地方自己去转换
            ...
            try {
               ...
               // Log.v("xmlrpc日期转换","原始:" + value + "  转换结果:" + obj.toString()); 
    ...
    static String getStartTag(String type){
    	return "<" + type + ">";
    }
    static String getEndTag(String type){
    	return "</" + type + ">";
    }
    static String escapedString(String content){

...
public class XMLRPCClientTest {
     ...
     */
    public XMLRPCClientTest(URI uri, String httpuser, String httppasswd) {
        ...
        if (uri.getScheme() != null){
            ...
            if(uri.getScheme().equals("https")) {
                if(uri.getPort() == -1)
                    try {
                    	 client = new ConnectionClientTest(creds, 443,httpParams,registry);
                    } catch (KeyManagementException e) {
                        client = new ConnectionClientTest(creds,httpParams,registry);
                        ...
                        try {
                            client = new ConnectionClientTest(creds, uri.getPort(),httpParams,registry);
                        } catch (KeyManagementException e) {
                            client = new ConnectionClientTest(creds,httpParams,registry);
                        } catch (NoSuchAlgorithmException e) {
                            client = new ConnectionClientTest(creds,httpParams,registry);
     ...
     */
    public XMLRPCClientTest(String url, String httpuser, String httppasswd) {
     ...
     */
    public XMLRPCClientTest(URL url, String httpuser, String httppasswd) {
        ...
        try {
            ...
            //synchronized (client) {
	            ...
	            if (method.equals("wp.uploadFile")){
	                ...
	                RequestParams paramtest = new RequestParams(Constants.MY_BABY_XMLRPC_URL);
		            paramtest.addHeader( "User-Agent", "mybaby-android/" + MyBaby.version);
		           ...
		           //paramtest.addBodyParameter(method,serializer); // 如果文件没有扩展名, 最好设置contentType参数.
		            x.http().post(paramtest, new Callback.CommonCallback<String>() {
		                ...
		                public void onError(Throwable ex, boolean isOnCallback) {
		                    Toast.makeText(x.app(), ex.getMessage(), 0).show();
		                }
